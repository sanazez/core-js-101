...
function isLeapYear(date) {
  ...
  if (new Date(year, 1, 29).getMonth() === 1) return true;
  return false;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const addZeroMillisec = (num) => {
    const str = `${num}`;
    if (str.length === 1) return `00${str}`;
    return str;
  };
  const date = new Date(endDate - startDate);
  return `${addZero(date.getUTCHours())}:${addZero(date.getMinutes())}:${addZero(date.getSeconds())}.${addZeroMillisec(date.getMilliseconds())}`;
}
